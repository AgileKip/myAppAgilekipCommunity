<template>
  <div>
    <h2 id="page-heading" data-cy="AuthorHeading">
      <span v-text="$t('myAppAgilekipCommunityApp.author.home.title')" id="author-heading">Authors</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('myAppAgilekipCommunityApp.author.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'AuthorCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('myAppAgilekipCommunityApp.author.home.createLabel')"> Create a new Author </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && authors && authors.length === 0">
      <span v-text="$t('myAppAgilekipCommunityApp.author.home.notFound')">No authors found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="authors">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('myAppAgilekipCommunityApp.author.name')">Name</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="myAppAgilekipCommunityApp.author.delete.question" data-cy="authorDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-author-heading" v-text="$t('myAppAgilekipCommunityApp.author.delete.question', { id: removeId })">
